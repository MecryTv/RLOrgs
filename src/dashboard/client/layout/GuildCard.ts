/**
 * Bausteine, die sich Serverliste und Serverdetail teilen: das Wappen und die
 * Mitgliederzahl. Sie liegen hier statt in einer der beiden Seiten, weil sonst
 * eine die andere importieren muesste - und die andere zurueck.
 */
import { IGuild } from "../interfaces/IGuild.js";
import { compact, numbers } from "../core/Format.js";

export function paintCrest(crest: HTMLElement, guild: IGuild): void {
...
export function countMembers(root: ParentNode, guild: IGuild): void {
    const members = root.querySelector<HTMLElement>("[data-members]")!;
    const bots = root.querySelector<HTMLElement>("[data-bots]");
    ...
    bots.hidden = false;
    bots.querySelector<HTMLElement>("[data-botcount]")!.textContent = compact(guild.bots);
