...
/* ----------------------------------------------------------
   Konto verknuepfen (Einstellungen)

   Genau eine Anmeldung: Epic. Steam, Xbox, PlayStation und Switch standen hier
   einmal als eigene Zeilen - das ist wieder weg. Fuer PlayStation und Nintendo
   gibt es keinen oeffentlichen Login, und die Raenge sind auf allen
   Plattformen dieselben; die Zeilen versprachen also eine Wahl, die es nicht
   gab. Siehe Migration 007.
   ---------------------------------------------------------- */
export interface IAccountsPayload {
    ready: boolean;
    epicLogin: boolean;
    account: { name: string | null; accountId: string; verified: boolean; linkedAt: string } | null;
    cooldown: { open: boolean; waitMs: number; until: string | null } | null;
    hint?: string;
}

/** Die eine Zeile: das Epic-Konto. */
export function accountRow(data: IAccountsPayload): HTMLElement {
...
export async function paintAccounts(): Promise<void> {
    const list = maybe<HTMLElement>("#acctList");
    const hint = maybe<HTMLElement>("#acctHint");
    const fallback = maybe<HTMLElement>("#acctFallback");

    if (!list || !hint || !fallback) return;
    ...
    const data = (await response.json()) as IAccountsPayload;
...
export function bindAccountFallback(): void {
    const found = {
        input: maybe<HTMLInputElement>("#acctEpic"),
        save: maybe<HTMLButtonElement>("#acctSave"),
        hint: maybe<HTMLElement>("#acctHint"),
    };

    if (!found.input || !found.save || !found.hint) return;
 ...
 */
export const EPIC_RESULTS: Record<string, [string, string]> = {
...
export function reportEpicReturn(): void {
    ...
    if (found) toast("info", found[0], found[1]);
