...
export function waitLabel(ms: number): string {
    ...
    if (ms < 60_000) return `${Math.ceil(ms / 1000)} Sekunden`;
    if (ms < 3_600_000) return `${Math.ceil(ms / 60_000)} Minuten`;
    if (ms < 86_400_000) return `${Math.ceil(ms / 3_600_000)} Stunden`;
