...
import { IActivity, IActivityPerson } from "../interfaces/IActivity.js";
...
type Metric = "messages" | "voice" | "members";
...
const HEAT_TITLES: Record<Metric, string> = {
...
function duration(minutes: number): string {
    if (minutes < 60) return `${numbers.format(minutes)} Min.`;
...
function niceMax(value: number): number {
    ...
    for (const step of [1, 2, 5]) if (value <= step * power) return step * power;
...
function hover(target: HTMLElement, read: () => [string, string]): void {
    const box = tip();
...
function tile(host: HTMLElement, label: string, key: string | null): ITile {
...
function trend(note: HTMLElement, now: number, before: number): void {
    if (before === 0) {
...
interface IDay {
    date: Date;
    /** null: vor der ersten gezählten Stunde - keine Daten, nicht null Aktivität. */
    values: number[] | null;
}

function daysOf(activity: IActivity, series: number[][]): IDay[] {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const days: IDay[] = Array.from({ length: DAYS }, (_, index) => {
    ...
    for (const day of days) {
        ...
        if (end.getTime() <= since) day.values = null;
    }

    return days;
}

function valueText(value: number, metric: Metric): string {
    if (metric === "voice") return duration(value);
    if (metric === "members") return count(value, "Beitritt", "Beitritte");

    return count(value, "Nachricht", "Nachrichten");
}

function dayTip(day: IDay, metric: Metric): [string, string] {
    const when = dayLabel(day.date);

    if (!day.values) return ["Keine Daten", `${when} · vor Beginn der Zählung`];
...
function bar(share: number, color: string): HTMLElement {
...
function columns(host: HTMLElement, days: IDay[], metric: Metric): void {
    ...
    const label = (value: number): string => (metric === "voice" ? duration(value * 60) : numbers.format(value));
    ...
    const ticks: [number, number][] = split
        ? [
              [100, max],
              [50, 0],
              [0, max],
    ...
    for (const day of days) {
        ...
        if (!day.values) column.classList.add("is-none");
    ...
    days.forEach((day, index) => {
        ...
        if ((days.length - 1 - index) % 7 !== 0) return;
...
function heatmap(host: HTMLElement, activity: IActivity, series: number[], metric: Metric): string {
    ...
    grid.forEach((hours, day) => {
        ...
        hours.forEach((value, hour) => {
            ...
            if (value === 0 || max === 0) cell.dataset.zero = "";
            else cell.style.setProperty("--v", (Math.ceil((value / max) * 5) / 5).toFixed(1));

            if (value > peak.value) peak = { value, day, hour };
...
/* ----------------------------------------------------------
   Listen
   ---------------------------------------------------------- */
function channels(host: HTMLElement, activity: IActivity): void {
    ...
    host.replaceChildren(
        ...activity.channels.map((channel) => {
            ...
            if (!channel.name) name.classList.add("is-empty");
...
function people(title: string, key: string, list: IActivityPerson[], format: (value: number) => string): HTMLElement {
    const box = document.createElement("div");
    ...
    for (const person of list) {
        ...
        if (!person.avatar) face.className = "toplists__face";

        const name = document.createElement("b");
        name.textContent = person.name ?? "Ehemaliges Mitglied";

        if (!person.name) name.classList.add("is-empty");
...
/* ----------------------------------------------------------
   Rang-Verteilung
   ---------------------------------------------------------- */
function ranks(host: HTMLElement, sub: HTMLElement, activity: IActivity, userId: string): void {
    const found = activity.ranks;

    if (!found) {
        sub.textContent = "";
        host.replaceChildren(
            hint("Dafür braucht der Bot die vollständige Mitgliederliste - das Members-Intent ist aus.")
        );
...
/* ----------------------------------------------------------
   Die Seite
   ---------------------------------------------------------- */
export type ActivityResult = { ok: true; activity: IActivity } | { ok: false; reason: "offline" | "error" };
 ...
 */
export async function fetchActivity(guildId: string): Promise<ActivityResult> {
    try {
        const response = await fetch(`${BASE}/api/guild/${encodeURIComponent(guildId)}/activity`, {
            headers: { Accept: "application/json" },
        });

        if (!response.ok) return { ok: false, reason: response.status === 503 ? "offline" : "error" };

        return { ok: true, activity: (await response.json()) as IActivity };
    } catch {
...
// ein Fehler.
function waitingHint(): void {
    for (const id of ["#ovChart", "#ovHeat", "#ovRanks", "#ovPeople"]) {
        need<HTMLElement>(id).replaceChildren(hint("Wird geladen …"));
    }
    ...
    need<HTMLElement>("#ovChannels").replaceChildren(row);
}
function bindMetric(activity: IActivity): void {
    const control = need<HTMLElement>("#ovMetric");
    const buttons = [...control.querySelectorAll<HTMLButtonElement>("button[data-metric]")];

    function show(metric: Metric): void {
        for (const button of buttons) button.setAttribute("aria-pressed", String(button.dataset.metric === metric));

        const series =
            metric === "voice" ? [activity.voice] : metric === "members" ? [activity.joins, activity.leaves] : [activity.messages];
        const days = daysOf(activity, series);
        const start = activity.since === null ? "" : ` · gezählt seit ${longDay(activity.since * HOUR)}`;

        need<HTMLElement>("#ovChartTitle").textContent = TITLES[metric];
        need<HTMLElement>("#ovChartSub").textContent = `Letzte ${DAYS} Tage${start}`;
        columns(need<HTMLElement>("#ovChart"), days, metric);

        const peak = heatmap(need<HTMLElement>("#ovHeat"), activity, series[0], metric);
        const heat = `${HEAT_TITLES[metric]} der letzten vier Wochen nach Wochentag und Uhrzeit, in deiner Zeitzone`;

        need<HTMLElement>("#ovHeatSub").textContent = peak ? `${heat} · ${peak}` : heat;
    }

    for (const button of buttons) {
        button.addEventListener("click", () => {
            show((button.dataset.metric ?? "messages") as Metric);
            clickSound("primary");
        });
    }
...
function week(activity: IActivity, joins: ITile, messages: ITile, voice: ITile): void {
    const length = activity.messages.length;
    const last = (series: number[]): number => sum(series.slice(length - WEEK));
    const before = (series: number[]): number => sum(series.slice(length - 2 * WEEK, length - WEEK));
...
export async function renderOverview(guild: IGuild, userId: string, waiting: Promise<ActivityResult>): Promise<void> {
    const host = need<HTMLElement>("#ovTiles");
    ...
    function note(text: string): void {
        const box = need<HTMLElement>("#ovNote");
    ...
    function fold(): void {
        for (const id of ["#ovTrend", "#ovWho", "#ovRanksCard"]) need<HTMLElement>(id).hidden = true;
    }
    ...
    bindMetric(activity);
    channels(need<HTMLElement>("#ovChannels"), activity);

    need<HTMLElement>("#ovPeople").replaceChildren(
        people("Im Chat", "var(--viz-chat)", activity.chatters, (value) => numbers.format(value)),
        people("Im Voice", "var(--viz-voice)", activity.talkers, duration)
    );

    ranks(need<HTMLElement>("#ovRanks"), need<HTMLElement>("#ovRanksSub"), activity, userId);
}
