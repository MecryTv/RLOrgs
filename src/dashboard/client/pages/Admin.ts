...
export interface IAdminPayload {
    me: { id: string; group: DashboardGroup };
    database: {
        ready: boolean;
        configured: boolean;
        name: string;
    ...
    bot: { tag: string | null; guilds: number; developerMode: boolean; uptime: number };
    totals: { teams: number; matches: number; accounts: number; settings: number } | null;
...
export const GRANTABLE: DashboardGroup[] = ["administrator", "partner", "premium"];

export function tile(label: string, value: string, hint?: string): HTMLElement {
...
export function paintTiles(data: IAdminPayload): void {
    const host = need<HTMLElement>("#adminTiles");
    ...
    const tiles = [
        ...
        tile(
            "Datenbank",
            database.ready ? "verbunden" : database.configured ? "nicht erreichbar" : "nicht eingerichtet",
            database.ready ? `${database.name} auf ${database.host}` : "siehe Log"
        ),
        tile(
            "Cache",
            `${numbers.format(cache.hits)} / ${numbers.format(cache.hits + cache.misses)}`,
            `Treffer · ${numbers.format(cache.size)} Einträge`
        ),
    ...
    if (totals) {
        tiles.push(
            tile("Aktive Teams", numbers.format(totals.teams)),
            tile("Partien", numbers.format(totals.matches)),
            tile("Verknüpfte Konten", numbers.format(totals.accounts)),
            tile("Eingerichtete Server", numbers.format(totals.settings))
        );
    }
...
export function paintEntries(data: IAdminPayload, onChange: () => void): void {
    const host = need<HTMLElement>("#entryList");

    need<HTMLElement>("#entryCount").textContent = String(data.entries.length);
...
export async function renderAdmin(): Promise<void> {
    const panel = need<HTMLElement>("#adminPanel");
    ...
    async function load(): Promise<void> {
        ...
        if (response.status === 403) {
            return fail(
                "Kein Zugriff auf die Administration",
                "Diese Seite steht nur Administratoren und Developern offen."
            );
        }

        if (!response.ok) return fail("Übersicht nicht ladbar", `Der Bot antwortete mit ${response.status}.`);

        const data = (await response.json()) as IAdminPayload;
        ...
        need<HTMLElement>("#adminMe").replaceChildren(icon(me.icon), me.label);
    ...
    const select = need<HTMLSelectElement>("#grantGroup");
    ...
    need<HTMLFormElement>("#grantForm").addEventListener("submit", (event) => {
        event.preventDefault();

        const id = need<HTMLInputElement>("#grantId");
        const note = need<HTMLInputElement>("#grantNote");

        void sendGroup(id.value.trim(), select.value as DashboardGroup, note.value.trim() || undefined, () => {
            id.value = "";
            note.value = "";
