...
export function bindFooter() {
    ...
    const site = maybe('meta[name="rlnexus-site"]')?.content.trim();
    const nav = maybe("#footerNav");
    if (site && nav)
        nav.prepend(link("", "Webseite", site));
    const bar = maybe("#cookieBar");
    const ok = maybe("#cookieOk");
    if (!bar || !ok)
        return;
