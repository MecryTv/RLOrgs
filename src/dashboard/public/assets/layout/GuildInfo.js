...
export function activeModules(guild) {
...
export function humansOf(guild) {
...
function popover() {
    ...
    const follow = () => {
        if (anchor && panel.matches(":popover-open"))
            place(panel, anchor);
    };
    ...
    panel.addEventListener("toggle", (event) => {
        ...
        if (closed && (document.activeElement === document.body || panel.contains(document.activeElement)))
            anchor?.focus();
    });
    pop = panel;
    return panel;
...
function head(guild) {
    ...
    return el("div", "ginfo__head", crest, el("div", "ginfo__title", el("b", "ginfo__name", guild.name), el("span", "ginfo__role", icon(ROLE_ICONS[guild.role]), ROLE_LABELS[guild.role])));
}
...
function content(guild) {
    ...
    const facts = [
        ...
        ...(guild.bots === null ? [] : [["Bots", numbers.format(guild.bots)]]),
        ["Server erstellt", monthOf(guild.created)],
        ...(guild.joined ? [["RL Nexus dabei seit", DAY.format(new Date(guild.joined))]] : []),
        ...(guild.teams ? [["Teams", numbers.format(guild.teams)]] : []),
    ];
    ...
    return [
        ...
        el("div", "ginfo__mods", el("b", "ginfo__label", modules.length ? `${modules.length} ${modules.length === 1 ? "Modul" : "Module"} aktiv` : "Noch kein Modul aktiv"), ...(modules.length ? [el("ul", "ginfo__modlist", ...modules.map((module) => el("li", "", icon(module.icon), el("span", "", module.name))))] : [])),
        el("p", "ginfo__rights", guild.canManage
...
export function showGuildInfo(guild, trigger) {
    const panel = popover();
    if (panel.matches(":popover-open") && anchor === trigger) {
        panel.hidePopover();
        return;
    }
    if (closedFor === trigger && performance.now() - closedAt < 400)
        return;
    anchor = trigger;
