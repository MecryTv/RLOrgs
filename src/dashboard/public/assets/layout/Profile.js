...
export const FALLBACK_AVATAR = "data:image/svg+xml;utf8," +
...
export function renderProfile(user) {
    const box = maybe("#avatar");
    const name = maybe("#profileName");
    const group = GROUPS[user.group] ?? GROUPS.testphase;
    if (box)
        box.replaceChildren(avatarOf(user));
    if (name)
    ...
    const label = maybe("#profileGroup");
    if (label) {
    ...
    if (profile)
        profile.setAttribute("aria-label", `Angemeldet als ${user.name}, ${group.label}`);
}
