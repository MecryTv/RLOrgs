...
function short(source) {
    ...
    if (source.startsWith("https://"))
        return source.replace(/^https:\/\//, "").slice(0, 40);
    return source.split("/").slice(1).join("/");
}
function blockCard(doc, index, context) {
    ...
    if (block.type === "section") {
        ...
        if (block.thumbnail) {
            row.append(chip(short(block.thumbnail), () => {
                block.thumbnail = "";
                context.onChange(true);
            }));
        }
...
export function renderEditor(host, doc, context) {
    ...
    for (const [type, label] of Object.entries(BLOCK_LABELS)) {
        const add = document.createElement("button");
        add.type = "button";
        add.className = "btn btn--quiet";
    ...
    for (let index = 0; index < doc.blocks.length; index++)
        list.append(blockCard(doc, index, context));
...
function inline(text, host, context) {
    for (const part of text.split(INLINE)) {
        ...
        if (part.startsWith("**") && part.endsWith("**"))
            wrap("strong", part.slice(2, -2));
        else if (part.startsWith("__") && part.endsWith("__"))
            wrap("u", part.slice(2, -2));
        else if (part.startsWith("*") && part.endsWith("*"))
            wrap("em", part.slice(1, -1));
        else if (part.startsWith("~~") && part.endsWith("~~"))
            wrap("s", part.slice(2, -2));
        else if (part.startsWith("`") && part.endsWith("`")) {
...
function markdown(text, context) {
    ...
    for (const raw of text.split("\n")) {
        ...
        const element = document.createElement(line.startsWith("-# ") ? "small" : heading ? `h${Math.min(Number(heading[1].length) + 1, 4)}` : "p");
...
function image(source, context, className) {
...
export function renderPreview(host, doc, context, extra) {
    const card = document.createElement("div");
    ...
    for (const block of doc.blocks) {
        ...
        if (block.type === "image") {
            ...
            for (const source of block.images)
                grid.append(image(source, context, "tkprev__image"));
            card.append(grid);
        }
        if (block.type === "section") {
