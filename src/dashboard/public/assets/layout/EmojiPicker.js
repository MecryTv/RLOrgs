...
function emojiUrl(id, animated, emojis) {
...
export function emojiNode(value, emojis, className = "emojiimg") {
...
function content(emojis, current, choose, insert) {
    ...
    function paint() {
        ...
        const cells = mode === "server"
            ...
            : STANDARD.filter(([emoji, words]) => !query || words.includes(query) || emoji === query).map(([emoji, words]) => cell(emoji, words.split(" ")[0], emojiNode(emoji, emojis)));
    ...
    take.addEventListener("click", () => {
        ...
        if (/\p{Extended_Pictographic}|\p{Regional_Indicator}/u.test(text))
            choose(text);
        else
            own.focus();
    });
