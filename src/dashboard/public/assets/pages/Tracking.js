...
export function paintClub(club, wtsi, ownName) {
    ...
    facts.append(clubFact("Owner", owner?.name ?? "—"), clubFact("Gegründet", club.createdAt ? new Date(club.createdAt).toLocaleDateString("de-DE") : "—"));
...
export function rates(stats) {
    const value = (key) => stats.find((stat) => stat.key === key)?.value ?? 0;
...
export async function renderTracking() {
    ...
    async function load() {
        ...
        if (data.stats.length > 0) {
            ...
            need("#statRates").replaceChildren(...rates(data.stats));
        }
