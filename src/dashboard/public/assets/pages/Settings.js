...
export function paintProfileCard(user) {
    const group = GROUPS[user.group] ?? GROUPS.testphase;
    const handle = user.handle ? `@${user.handle}` : "Discord-Konto";
    need("#idAvatar").replaceChildren(avatarOf(user));
    need("#idName").textContent = user.name;
    need("#idHandle").textContent = handle;
    need("#idGroup").replaceChildren(icon(group.icon), group.label);
    need("#idUser").textContent = user.id;
    need("#idGroupText").textContent = group.grants;
    const mail = need("#idMail");
    if (user.email) {
    ...
    need("#copyId").addEventListener("click", (event) => {
        ...
        function select() {
            ...
            range.selectNodeContents(need("#idUser"));
...
export function bindSettings() {
    const sound = need("#setSound");
    const volume = need("#setVolume");
    const label = need("#volumeLabel");
    const motion = need("#setMotion");
    const toasts = need("#setToasts");
    function paint() {
    ...
    toasts.addEventListener("change", () => {
        ...
        savePrefs();
        if (prefs.toasts)
            toast("info", "Hinweise sind an", "Meldungen erscheinen wieder unten rechts.");
    });
}
/* ----------------------------------------------------------
   Seite: Einstellungen
...
export function bindTabs() {
    const nav = maybe("#setNav");
    if (!nav)
        return;
    const links = [...nav.querySelectorAll("a[href]")].filter((link) => (link.getAttribute("href") ?? "").startsWith("#"));
    const cards = links
        .map((link) => maybe(link.getAttribute("href")))
...
export async function renderSettings(user) {
    const panel = need("#setPanel");
    const group = GROUPS[user.group] ?? GROUPS.testphase;
    need("#setAvatar").replaceChildren(avatarOf(user));
    ...
    need("#setMeta").replaceChildren(who, badge);
    need("#setTracker").href = `${BASE}/user/${user.id}/tracking`;
    need("#acctDiscord").textContent = user.handle ? `@${user.handle}` : user.name;
    paintProfileCard(user);
    bindTabs();
    bindSettings();
    bindAccountFallback();
    need("#empty").classList.remove("is-on");
    panel.hidden = false;
