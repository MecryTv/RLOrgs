...
export function need(selector) {
    ...
    if (!element)
        throw new Error(`Element ${selector} fehlt im Dokument.`);
    return element;
}
export function maybe(selector) {
    return document.querySelector(selector);
}
export function clone(id) {
    const template = need(id);
    const first = template.content.firstElementChild;
    if (!first)
        throw new Error(`Vorlage ${id} ist leer.`);
...
export function link(className, label, href, external = false) {
...
export function ghost(symbol, label) {
...
// wäre erfunden.
export function picture(source, className) {
