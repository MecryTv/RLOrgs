...
- **Der gebaute Dashboard-Client ist mitversioniert.** `tsc -p src/dashboard/tsconfig.json` schreibt nach `src/dashboard/public/assets/**`, und diese Dateien stehen in Git (`.gitignore` nimmt nur `/dist` aus). Wer eine Datei unter `src/dashboard/client/**` ändert, baut mit `npm run build:dashboard` und **committet das Ergebnis mit**. `npm start` baut nicht — ein Deploy ohne das Kompilat serviert sonst den alten Stand.
